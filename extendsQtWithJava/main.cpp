...
static jstring getQtVersion(JNIEnv *env, jobject thiz)
{
    ...
    //qDebug() << "qVersion()2:" << version;
    //QAndroidJniObject javaString = QAndroidJniObject::fromString(version);
    //qDebug() << "javaString1:" << javaString.object<jstring>();
    //qDebug() << "javaString2:" <<env->GetStringUTFChars(javaString.object<jstring>(), 0);
...
//     puts(msg);
     result = env->NewStringUTF(qVersion()); // C style string to Java String
     //qDebug() << "result:" << javaString.object<jstring>();
     return result;
...
static void onFileManager(JNIEnv *env, jobject thiz,int result, jstring fileUrl)
{
     qDebug() << "onFileManager, result - " << result << " fileUrl - " << fileUrl;
    QString image;
    const char *nativeString = env->GetStringUTFChars(fileUrl, 0);
    image = nativeString;
    env->ReleaseStringUTFChars(fileUrl, nativeString);
    qDebug() << "onFileManager, result - " << result << " image - " << image;
    int ret = result;
    if(result == -1 && QFile::exists(image))
    {
        ret = 3;
...
bool registerNativeMethods()
{
    JNINativeMethod methods[] {
        ...
        {"onFileManager", "(ILjava/lang/String;)V", (void*)onFileManager}
    };
