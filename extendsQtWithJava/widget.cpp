    ...
    : QWidget(parent)
{
    ...
    layout->addWidget(new QLabel("manager:"), 7, 0);
    m_fileManagerEdit = new QLineEdit("filePath");
    layout->addWidget(m_fileManagerEdit, 7, 1);
    m_fileManagerButton = new QPushButton("open");
    connect(m_fileManagerButton, SIGNAL(clicked()),
                      this, SLOT(onFileManager()));
    layout->addWidget(m_fileManagerButton, 7, 2);
...
bool Widget::event(QEvent *e)
{
    if(e->type() == SimpleCustomEvent::eventType())
    {
        ...
        qDebug()<<"Widget::event:"<<sce->m_arg1<<sce->m_arg2;
        if(sce->m_arg1 == 1)
        {
        ...
        else if(sce->m_arg1 == 3)
        {

            m_fileManagerEdit->setText(sce->m_arg2);
        }
        else
        {
...
void Widget::startActivity(QString name)
{
    ...
    QAndroidJniObject::callStaticMethod<void>("an/qt/extendsQtWithJava/ExtendsQtWithJava",
                                       "launchActivity",
                                       "(Ljava/lang/String;)V",//(Ljava/lang/String;)括号里是参数类型，括号右侧是返回值类型，V为void
                                       javaAction.object<jstring>());
#endif
}


void Widget::onFileManager()
{
#ifdef ANDROID
    //QAndroidJniObject javaAction = QAndroidJniObject::fromString(name);
    QAndroidJniObject::callStaticMethod<void>("an/qt/extendsQtWithJava/ExtendsQtWithJava",
